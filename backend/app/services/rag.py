...
inline like [1], [2] matching their number. If the excerpts don't contain the answer, say \
you don't have that information in the saved manuals instead of guessing."""


def retrieve(query: str, item_id: int | None = None, top_k: int = TOP_K) -> list[dict]:
    embedding = llm.embed_texts([query])[0]
    collection = get_manuals_collection()
    where = {"item_id": item_id} if item_id is not None else None
